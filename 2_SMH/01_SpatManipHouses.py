...
def run_trials(items, trial_order, practice=False):
    ...
    for i in trial_order:
        ...
        stim_left = visual.ImageStim(exp_win, image='%s/%s/stimuli/%s%s'%(PATH, LANGUAGE, item_prefix, item[7]), pos=[-200,0], units=u'pix')
        stim_right = visual.ImageStim(exp_win, image='%s/%s/stimuli/%s%s'%(PATH, LANGUAGE, item_prefix, item[8]), pos=[200,0], units=u'pix')
